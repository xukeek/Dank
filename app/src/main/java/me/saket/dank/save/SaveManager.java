...
import me.saket.dank.vote.VoteDirectionX;
...
public class SaveManager {
  ...
  public SaveManager(
      ...
      Lazy<Reddit> reddit,
      @Named("saves") Lazy<SharedPreferences> sharedPrefs,
      Lazy<Moshi> moshi)
  {
  ...
  @CheckResult
  public Completable saveAndSend(Save save) {
    // Mark the save as pending immediately so that getPendingVote() can be used immediately after calling save().
    markVoteAsPending(save.contributionToVote(), save.saved());

    return save.sendToRemote(reddit.get())
...
//        .doOnComplete(() -> Timber.i("Voting done for %s", contributionToVote.fullName()()))
   ...
   */
  @CheckResult
  public Completable voteWithAutoRetry(Save save) {
    return saveAndSend(save)
        .onErrorComplete(error -> {
          ...
          if (isTooManyRequestsError(error) || !Dank.errors().resolve(error).isUnknown()) {
            ...
            Timber.i("Save failed for %s. Will retry again later. Error: %s", save.contributionToVote().getFullName(), error.getMessage());
            SaveJobService.scheduleRetry(appContext, save.contributionToVote(), save.saved(), moshi.get());
            shouldComplete = true;
...
//  }
  ...
  public boolean isSavePending(Identifiable identifiable) {
    return sharedPrefs.get().contains(keyFor(identifiable));
  }

  private void markVoteAsPending(Identifiable votableContribution, boolean saved) {
    sharedPrefs.get().edit().putBoolean(keyFor(votableContribution), saved).apply();
  }
  ...
  public <T extends Votable & Identifiable> int getScoreAfterAdjustingPendingVote(T votableContribution) {
    if (!isSavePending(votableContribution)) {
